...
log_path = Path(Path.cwd(), 'fp_inspector/obfuscation/js_obfuscator.log')
...
for site in tqdm(sites):
    ...
    js_obfuscator_path = Path.home().joinpath(obfuscator_path, 'js_obfuscator')
    js_obfuscator_path.mkdir(exist_ok=True)
    ...
    if os.path.exists(js_path):
        ...
        for script in scripts:
            ...
            output_obfuscated = Path(js_obfuscator_path, script_name)
            print(output_obfuscated)

            if not os.path.exists(output_obfuscated):
                command = ['yarn', 'run', 'javascript-obfuscator', script, '--output', output_obfuscated]
