            ...
            if not any(scriptURL.startswith(invalid) for invalid in INVALID_URLS):
                try:
                    ...
                    if functionName:
                        ...
                        # Ensure all nested levels of dictionaries are properly handled
                        site = db.setdefault(siteName, {})
                        script = site.setdefault(scriptURL, {})
                        functionDict = script.setdefault(scriptId, {})
                        current_entry = functionDict.get(functionName, {"API_args": [], "FP": False, "Bytecode": []})
                        current_entry["Bytecode"] = bytes
    ...
    for trace in apiTraces:
        ...
        try:
            ...
            if not functionName:
                ...
                print("Process_apis: anonymous function: ", functionName)
            ...
            if functionName not in db[siteName][scriptURL][scriptId]:
                if functionName.startswith("anonymous_func_"):
                    ...
                    print("anonymous_func_")
                    bytes = ['Parameter count 0', 'Register count 0', 'Frame size 0','anonymous_bytes']
