        ...
        if batch != []:
            ...
            if not any(scriptURL.startswith(invalid) for invalid in INVALID_URLS):
                try:
                    ...
                    if scriptURL in db[siteName]:
                        if scriptId in db[siteName][scriptURL]:
                            if functionName != 'Function name:' and functionName != '':
                                if functionName in db[siteName][scriptURL][scriptId]:
                                    db[siteName][scriptURL][scriptId][functionName]["Bytecode"] = bytecodes
                                # else:
                                #     db[siteName][scriptURL][scriptId][functionName] = {"API_args": [], "FP": False, "Bytecode": bytes}
                    #     else: 
                    #         if scriptURL != "" and scriptURL != "No URL":
                    ...
                    #             db[siteName][scriptURL][scriptId][functionName] = {"API_args": [], "FP": False, "Bytecode": ""}
                    #         db[siteName][scriptURL][scriptId][functionName]["Bytecode"] = bytes
                    # else:
                    #     if scriptURL != "" and scriptURL != "No URL":
                    #         db[siteName][scriptURL][scriptId] = {}
                    #     if functionName == 'Function name:' and functionName == '':
...
def process_api_traces(apiTraces: List[str], db: Dict[str, Any], siteName: str, log_path: Path):
    ...
    for trace in apiTraces:
        ...
        try:
            ...
            # Create nested dictionary structure if not already present.
...
def main(chunk):
    ...
    print(CHUNK_NAME)
    ##### test
    # if CHUNK_NAME == "chunk8":
    database_path = Path(Path.cwd(), f"create_DataBase/DB/function_level/DB_{CHUNK_NAME}.json")
    # if not os.path.exists(database_path):
    log_path = Path(Path.cwd(), f"create_DataBase/DB/function_level/logs_DB/log_{CHUNK_NAME}.txt")
